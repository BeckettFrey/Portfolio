...
import type { AboutConfig, InterestItem, SocialLink } from './types';
...
const AboutContent = ({ config, glassPanel = 'bg-white/10 backdrop-blur-md' }: AboutContentProps) => {
  ...
  const renderInterestItem = (item: InterestItem) => {
    ...
    const colorClass = COLOR_CLASSES[item.color as keyof typeof COLOR_CLASSES];
    if (!IconComponent || !colorClass) return null;

    return (
      <div key={item.text} className={`flex items-center p-3 ${glassPanel} rounded-xl`}>
  ...
  return (
    <>
      {/* Profile Section */}
      <div className={`${glassPanel} rounded-3xl shadow-xl p-8 mb-8 border border-white/10`}>
        <div className="grid md:grid-cols-3 gap-8 items-center">
          <div className="md:col-span-2">
            <h2 className="text-3xl font-bold text-white mb-4">{config.greeting}</h2>
            <div className="text-lg text-gray-300 leading-relaxed mb-6 space-y-4">
              {introText.split('\n\n').map((para, idx) => <p key={idx}>{para}</p>)}
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-center">
              <div className={`${glassPanel} p-4 rounded-xl`}>
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-8">
        <div className={`${glassPanel} rounded-3xl shadow-xl p-8 border border-white/10`}>
