...
export default function ProjectsPanel({ config }: Props) {
  const [filterType, setFilterType] = useState('all');
  const [filterTech, setFilterTech] = useState('all');
  const [filterLang, setFilterLang] = useState('all');

  const allTechs = Array.from(new Set(config.featured_projects.flatMap(p => p.tech_stack || [])));
  const allLangs = Array.from(new Set(config.featured_projects.flatMap(p => p.languages || [])));

  const filteredProjects = config.featured_projects.filter(project => {
    if (filterType !== 'all') {
  ...
  const uniqueLanguages = Array.from(new Set(config.featured_projects.flatMap(p => p.languages || [])));
  ...
  return (
    <div className="relative text-white font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {[
          { count: config.featured_projects.length, label: 'Projects', color: 'blue' },
          ...
          >
            {project.highlight && (
              <div className="bg-gradient-to-r from-blue-400 to-purple-400 text-white text-xs font-bold px-3 py-1 text-center">
                {config.highlightBadge || '⭐ FEATURED PROJECT'}
              </div>
            )}
            ...
            <div className="p-6 pb-4 flex-grow flex flex-col">
              ...
              <p className="text-gray-300 text-sm leading-relaxed mb-4 flex-grow">
                {project.custom_description || config.noDescription || 'No description available'}
              </p>
              {Array.isArray(project.tech_stack) && project.tech_stack.length > 0 && (
                <div className="mb-4">
                  <p className="text-xs font-medium text-gray-400 mb-2">
                    {config.techStackLabel || 'Tech Stack:'}
                  </p>
      ...
      {filteredProjects.length === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="text-2xl font-bold text-red-400 mb-4">404</h3>
